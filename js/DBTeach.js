class TeachInfoPage extends TeacherInfoPage {
    static TblName = 'teach_relation';
    constructor(tblname) {
        super(tblname);
    }
    display() {
        TeachInfoPage.clean();
        TeachInfoPage.DisplayData();
        this.buildInsertDiv();
        this.buildQueryDiv();
        this.buildDeleteDiv();
    }
    ...
    static DisplayData() {
        var tblNameList = ["teach_relation", "teacher", "course"];
        var JoinClause = ["teach_relation.cno", "course.id", "teach_relation.tno", "teacher.id"];
        queryWithNaturalJoin(tblNameList, JoinClause, '');

        var tabledata = QueryResult.get(TeachInfoPage.TblName);
        ...
        this.InfoTable = generateTable2D(tabledata.length, tabledata[0].length, tabledata, TeachInfoPage.TblName);
    ...
    static DisplayQueryResult() {
        ...
        var tabledata = QueryResult.get(TeachInfoPage.TblName);
        if (tabledata == null || tabledata.length == 0) return;
        this.QueryTable = generateTable2D(tabledata.length, tabledata[0].length, tabledata, TeachInfoPage.TblName);
    ...
    buildInsertDiv() {
        ...
        button.onclick = function () {
            ...
            var colnames = ColumnInfo.get(TeachInfoPage.TblName);
            ...
            insertIntoTable(TeachInfoPage.TblName, data_map);
            TeachInfoPage.DisplayData();
        }//function
        button.width = "30px";
        button.innerHTML = "提交";
    ...
    buildQueryDiv() {
        ...
        button.onclick = function () {
            ...
            for (var i in children) {
                if (children[i].localName == 'input') {
                    ...
                    if (TextToColumnName[placeholder] == null)
                        columnname = TeachInfoPage.TblName + "." + colnames[index];
                    else
            ...
            TeachInfoPage.DisplayQueryResult();
        }//function
        button.width = "30px";
        button.innerHTML = "提交";
    ...
    buildDeleteDiv() {
        ...
        button.onclick = function () {
            ...
            deleteFromTable(TeachInfoPage.TblName, data_map);
            TeachInfoPage.DisplayData();
        }//function
        button.width = "30px";
        button.innerHTML = "提交";

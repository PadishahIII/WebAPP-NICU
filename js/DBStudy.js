class StudyInfoPage extends TeacherInfoPage {
    static TblName = 'study_relation';
    constructor(tblname) {
        super(tblname);
    }
    display() {
        StudyInfoPage.clean();
        StudyInfoPage.DisplayData();
        this.buildInsertDiv();
        this.buildQueryDiv();
        this.buildDeleteDiv();
    }
    ...
    static DisplayData() {
        var tblNameList = ["study_relation", "course", "student"];
        var JoinClause = ["study_relation.cno", "course.id", "study_relation.sno", "student.id"];
        queryWithNaturalJoin(tblNameList, JoinClause, '');

        var tabledata = QueryResult.get(StudyInfoPage.TblName);
        ...
        this.InfoTable = generateTable2D(tabledata.length, tabledata[0].length, tabledata, StudyInfoPage.TblName);
    ...
    static DisplayQueryResult() {
        ...
        var tabledata = QueryResult.get(StudyInfoPage.TblName);
        if (tabledata == null || tabledata.length == 0) return;
        this.QueryTable = generateTable2D(tabledata.length, tabledata[0].length, tabledata, StudyInfoPage.TblName);
    ...
    buildInsertDiv() {
        ...
        button.onclick = function () {
            ...
            var colnames = ColumnInfo.get(StudyInfoPage.TblName);
            ...
            insertIntoTable(StudyInfoPage.TblName, data_map);
            StudyInfoPage.DisplayData();
        }//function
        button.width = "30px";
        button.innerHTML = "提交";
    ...
    buildQueryDiv() {
        ...
        button.onclick = function () {
            ...
            for (var i in children) {
                if (children[i].localName == 'input') {
                    ...
                    if (TextToColumnName[placeholder] == null)
                        columnname = StudyInfoPage.TblName + "." + colnames[index];
                    else
            ...
            StudyInfoPage.DisplayQueryResult();
        }//function
        button.width = "30px";
        button.innerHTML = "提交";
    ...
    buildDeleteDiv() {
        ...
        button.onclick = function () {
            ...
            deleteFromTable(StudyInfoPage.TblName, data_map);
            StudyInfoPage.DisplayData();
        }//function
        button.width = "30px";
        button.innerHTML = "提交";
